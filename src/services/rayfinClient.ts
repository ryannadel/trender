...
let client: RayfinClient<TrenderSchema> | null = null;
let localDev = false;

export function initRayfinClient(
  config: RayfinClientConfig
): RayfinClient<TrenderSchema> {
  if (client) {
    throw new Error('Rayfin client is already initialized.');
  }
  client = new RayfinClient<TrenderSchema>({
...
export function getRayfinClient(): RayfinClient<TrenderSchema> {
  if (!client) {
    throw new Error(
